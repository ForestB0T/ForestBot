    ...
    execute: async (user: string, args: any[], bot: Bot) => {
        ...
        switch (choice) {
            case 'kills':
                ...
                const stringKills: string = Kills.map((element: any) => `${element.username}: ${element.stat}`).join(", ");
            ...
            case 'deaths':   
                ...
                const stringDeaths: string = Deaths.map((element: any) => `${element.username}: ${element.stat}`).join(", ");
            ...
            case 'joins':
                ...
                const stringJoins: string = Joins.map((element: any) => `${element.username}: ${element.stat}`).join(", ");
            ...
            case 'playtime':            
                ...
                const stringPlaytime: string = Playtime.map((element: any) => `${element.username}: ${Math.floor(element.stat / (1000 * 60 * 60 * 24))} Days`).join(", ");
